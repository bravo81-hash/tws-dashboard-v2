...
# This function will be the target for our TWS thread
def run_tws_connection(app_instance: FastAPI):
    """Initializes, connects, and runs the TWS connection loop."""
    ib_app = IBKRApp()
    app_instance.state.tws_app = ib_app
    ib_app.connect(settings.TWS_HOST, settings.TWS_PORT, clientId=settings.TWS_CLIENT_ID)
    ib_app.run() # This is the blocking call that runs the TWS message loop
...
async def lifespan(app: FastAPI):
    ...
    print("INFO:     Starting TWS connection in a background thread...")
    
    # Start one single, simple thread for all TWS activity.
    tws_thread = threading.Thread(target=run_tws_connection, args=(app,), daemon=True)
    tws_thread.start()
    
    # Give it a moment to establish the connection before proceeding
    ...
    if hasattr(app.state, 'tws_app'):
        app.state.tws_app.disconnect()
...
class Position(BaseModel):
    ...
    greeks: dict
    status: str
class ComboWithAnalytics(BaseModel):
    id: Optional[int]
    name: str
    createdAt: datetime
    dte: int
...
@app.get("/")
def read_root():
    return {"message": "TWS Dashboard Backend is running!"}
@app.get("/portfolio", response_model=List[Position])
def get_portfolio():
    return portfolio_manager.get_all_positions()
@app.post("/combos", response_model=Combo)
def create_combo(combo: ComboCreate, session: Session = Depends(get_session)):
    db_combo = Combo.model_validate(combo)
...
def read_combos(session: Session = Depends(get_session)):
    ...
    for db_combo in combos_from_db:
        ...
        for leg_position in combo_legs_data:
            ...
            combo_analytics.delta += (leg_greeks.get('delta', 0.0) or 0) * pos_size * multiplier
            combo_analytics.gamma += (leg_greeks.get('gamma', 0.0) or 0) * pos_size * multiplier
            combo_analytics.vega += (leg_greeks.get('vega', 0.0) or 0) * pos_size * multiplier
            combo_analytics.theta += (leg_greeks.get('theta', 0.0) or 0) * pos_size * multiplier
...
def calculate_pnl_curve(legs: List[dict], price_range: np.ndarray, days_to_add: int):
    ...
    for leg in legs:
        ...
        cost_basis = leg.get('costBasis', 0.0)
        
        if ' C' in desc or ' P' in desc:
            parts = desc.split(' ')
            expiry_str, strike_str, right = parts[1], parts[2], parts[3]
            K = float(strike_str)
            flag = right.lower()
            ...
            if T_sim > 1e-9:
                ...
                if iv is None or iv <= 0: iv = 0.25 
                option_prices = np.asarray( vectorized_black_scholes(flag, price_range, K, T_sim, settings.RISK_FREE_RATE, iv) )
...
def get_risk_profile(request: RiskProfileRequest):
    ...
    if not combo_legs:
        raise HTTPException(status_code=404, detail="No valid legs found")
    und_price = next((leg.get('greeks', {}).get('undPrice') for leg in combo_legs if leg.get('greeks', {}).get('undPrice') is not None), None)
    if not und_price:
        pos = combo_legs[0].get('position', 0)
        und_price = combo_legs[0].get('marketValue', 0) / pos if pos != 0 else 0
    if not und_price:
         raise HTTPException(status_code=404, detail="Could not determine underlying price")
...
def get_3d_surface(request: RiskProfileRequest):
    ...
    price_axis = np.linspace(und_price * 0.75, und_price * 1.25, 40)
    exp_dates = [datetime.strptime(leg.get('description', '').split(' ')[1], "%Y-%m-%d") for leg in combo_legs if ' C' in leg.get('description', '') or ' P' in leg.get('description', '')]
    min_exp_date = min(exp_dates) if exp_dates else datetime.now()
    current_dte = (min_exp_date - datetime.now()).days
    time_axis = np.linspace(max(0, current_dte), 0, 30, dtype=int)
    pnl_surface = []
    ...
    return Surface3DResponse(price_axis=price_axis.tolist(), time_axis=time_axis.tolist(), pnl_surface=pnl_surface)